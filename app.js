(function () {
  ...
  function csvRow(name, type, unit, value, linkedVariable) {
    return [name, type, unit || '', value, linkedVariable || ''];
  }
  ...
  function buildColorScale(baseHex, prefix) {
    ...
    return values.map(function (step) {
      ...
      return csvRow(prefix + '/' + step, 'Color', '', colorValue, '');
    });
  }

  function buildNeutralScale(name, solidHex) {
    const colorChannels = name === 'White' ? '255,255,255' : '0,0,0';
    const scale = [
      csvRow('Color/' + name + '/100', 'Color', '', solidHex, '')
    ];
    ...
    for (let step = 90; step >= 10; step -= 10) {
      scale.push(
        csvRow(
          ...
          'Color',
          '',
          'rgba(' + colorChannels + ',' + (step / 100).toFixed(1) + ')',
          ''
        )
      );
    }
  ...
  function buildRows() {
    ...
    const rows = [['Name', 'Type', 'Unit', 'Value', 'Linked Variable']];
    ...
    rows.push(csvRow('Font/Display', 'Font', '', displayFont, ''));
    rows.push(csvRow('Font/Body', 'Font', '', bodyFont, ''));

    const typeScale = [
      { label: 'Display', multiplier: 3.5, height: '1.1', tracking: '-0.02' },
      { label: 'H1', multiplier: 2.5, height: '1.2', tracking: '-0.01' },
      { label: 'H2', multiplier: 2.0, height: '1.2', tracking: '-0.01' },
      { label: 'H3', multiplier: 1.5, height: '1.2', tracking: '-0.01' },
      { label: 'Body', multiplier: 1.0, height: '1.5', tracking: '0' },
      { label: 'Small', multiplier: 0.875, height: '1.5', tracking: '0' }
    ];

    typeScale.forEach(function (entry) {
      rows.push(csvRow('Text/' + entry.label + '/Size', 'Size', 'px', formatSizeValue(baseSize * entry.multiplier), ''));
      rows.push(csvRow('Text/' + entry.label + '/Height', 'Number', '', entry.height, ''));
      rows.push(csvRow('Text/' + entry.label + '/Tracking', 'Size', 'em', entry.tracking, ''));
    });

    [4, 8, 16, 24, 32, 48, 64, 96].forEach(function (space) {
      rows.push(csvRow('Space/' + space, 'Size', 'px', String(space), ''));
    });

    rows.push(csvRow('Radius/SM', 'Size', 'px', '4', ''));
    rows.push(csvRow('Radius/MD', 'Size', 'px', '8', ''));
    rows.push(csvRow('Radius/LG', 'Size', 'px', '16', ''));
    rows.push(csvRow('Radius/Round', 'Size', 'px', '999', ''));
